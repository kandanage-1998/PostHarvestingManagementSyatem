...
{
    public class CropManagementService : ICropManagementService
    {
        ...
        public CropManagementService(IPostHarvestManagementUnitOfWork UnitOfWork, IPostHarvestManagementResponse PostHarvestManagementResponse, IConfiguration configuration)
        {
            this.PostHarvestManagementResponse = PostHarvestManagementResponse;
            this.UnitOfWork = UnitOfWork;
            this.configuration = configuration;
        }
        public async Task<PostHarvestManagementResponse> GetCropManagementCollectionPointWise(CropDemandInputModel model)
        {
            try
            {
                ...
                var result = await UnitOfWork.Repository<CropManagementOutputModel>().GetEntitiesBySPAsync("[Administration].[GetCropManagementCollectionPointWise]", parameters);
                if (result.Count() > 0)
                {
                    return PostHarvestManagementResponse.GenerateResponseMessage(PostHarvestManagementResponseEnum.Success.ToString(), "Crop Management Sucessfully", result);
                }
                else
                {
                    return PostHarvestManagementResponse.GenerateResponseMessage(PostHarvestManagementResponseEnum.Error.ToString(), "Crop Management Failed", result);
                }
