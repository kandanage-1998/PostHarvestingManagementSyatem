...
export default function SellerRegistration() {
    ...
    const DonationRequestSchema = Yup.object().shape({
        ...
        sellingKilos: Yup.number().required('Selling Kilos Required').positive('Kilos must be a positive number'),
        sellingPurpose: Yup.string().required('Selling Purpose Required'),
        cropRemovePurpose: Yup.string(),
    ...
    function handleEdit(index) {
        ...
        setValues({
            ...
            sellingKilos: selectedRecord.sellingKilos,
            sellingPurpose: selectedRecord.sellingPurpose,
            cropRemovePurpose: selectedRecord.cropRemovePurpose,
    ...
    async function handleSave() {
        if (editIndex !== null) {
            ...
            //const result = await axios.get('https://localhost:7211/api/CropManagementRegistration/SaveCropManagementRegistration', model);
            const result = await axios.post('https://localhost:7211/api/CropManagementRegistration/SaveCropManagementRegistration', model);
    ...
    return (
        <Box>
            <Helmet>
                <title> CropManagementRegistration | PostHarvestManagement </title>
            </Helmet>
            <Container maxWidth="xl">
                <Typography variant="h4" sx={{ mb: 5 }}>
                    Crop Management Registration
                </Typography>
                        ...
                        <Stack spacing={2}>
                            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                                ...
                                <TextField
                                    ...
                                    label="Selling Kilos *"
                                    type="number"
                                    {...getFieldProps('sellingKilos')}
                                    error={Boolean(touched.sellingKilos && errors.sellingKilos)}
                                    helperText={touched.sellingKilos && errors.sellingKilos}
                                />
                                ...
                                <TextField
                                    ...
                                    fullWidth
                                    size="small"
                                    label="Selling Purpose *"
                                    value={values.sellingPurpose}
                                    onChange={formik.handleChange}
                                    {...getFieldProps('sellingPurpose')}
                                    error={Boolean(touched.sellingPurpose && errors.sellingPurpose)}
                                    helperText={touched.sellingPurpose && errors.sellingPurpose}
                                >
                                    <MenuItem value="Vegetable Shops">Vegetable Shops</MenuItem>
                                    <MenuItem value="Supermarkets">Supermarkets</MenuItem>
                                    <MenuItem value="Export Foreign Countries">Export Foreign Countries</MenuItem>
                                    <MenuItem value="Food Processing Companies">Food Processing Companies</MenuItem>
                                    <MenuItem value="Other">Other</MenuItem>
                                    <MenuItem value="Not selling">Not selling</MenuItem>
                                </TextField>

                                {/* Conditional rendering for Crop Remove Purpose */}
                                {values.sellingPurpose === "Not selling" && (
                                    <TextField
                                        select
                                        fullWidth
                                        size="small"
                                        label="Crop Remove Purpose"
                                        value={values.cropRemovePurpose} 
                                        onChange={formik.handleChange}
                                        {...getFieldProps('cropRemovePurpose')}
                                        error={Boolean(touched.cropRemovePurpose && errors.cropRemovePurpose)}
                                        helperText={touched.cropRemovePurpose && errors.cropRemovePurpose}
                                    >
                                        <MenuItem value="For Fertilizer">For Fertilizer</MenuItem>
                                        <MenuItem value="For Animal Farms">For Animal Farms</MenuItem>
                                        <MenuItem value="For Elephant Food">For Elephant Food</MenuItem>
                                        <MenuItem value="Other">Other</MenuItem>
                                    </TextField>
                                )}
                    ...
                    <Table>
                        <TableHead>
                            <TableRow>
                                ...
                                <TableCell>Selling Kilos</TableCell>
